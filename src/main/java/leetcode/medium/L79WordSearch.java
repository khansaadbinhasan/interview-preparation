...
public class L79WordSearch {
    ...
    public boolean exist(char[][] board, int i, int j, String word, int k, boolean[][] visited) {
        if( k >= word.length() ) return true;
        if( i >= board.length || j >= board[0].length || i < 0 || j < 0 || visited[i][j] ) return false;

        visited[i][j] = true;

        if( board[i][j] == word.charAt(k) ) {
            if(  exist(board, i+1, j, word, k+1, visited) ||
                    exist(board, i, j+1, word, k+1, visited) || exist(board, i, j-1, word, k+1, visited) ||
                    exist(board, i-1, j, word, k+1, visited) ) return true;
        }
