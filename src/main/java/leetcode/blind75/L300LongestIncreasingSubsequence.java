...
public class L300LongestIncreasingSubsequence {
    ...
    boolean[] visited;//t,f,f,f,f
    int[] maxArr2;//3,0,2,2,1

    public int lengthOfLISDFS(int[] nums) {
        maxArr2 = new int[nums.length];
    ...
    public int dfs(int[] nums, int i){
        ...
        if( maxArr2[i] != 0 ) return maxArr2[i];
        ...
        for( int j = i+1; j < nums.length; j++ ){
            if( nums[j] > nums[i] ) maxLen = Math.max( maxLen, dfs(nums, j) );
        }

        visited[i] = false;

        return maxArr2[i] = (maxLen+1);
    }
