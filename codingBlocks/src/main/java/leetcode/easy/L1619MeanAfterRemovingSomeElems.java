...
public class L1619MeanAfterRemovingSomeElems {

    public double trimMean(int[] arr) {

        int numElemsToRemove = arr.length / 20;

        Arrays.sort(arr);

        double sum = 0;

        for( int i = numElemsToRemove; i < arr.length - numElemsToRemove; i++ ){
            sum += arr[i];
        }

        return sum / (arr.length - numElemsToRemove*2);
    }
